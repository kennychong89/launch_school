...
def play_a_round_of_21
  ...
  if !dealer_wins?(score_tracker) && !dealer_reached_hit_limit?(get_score(score_tracker, DEALER_NAME))
    dealer_turn(dealer_hand, player_hand, score_tracker, deck)
  end
  display_round_results(score_tracker)
  get_winner_name(score_tracker) unless is_tied(score_tracker)
end
...
def calculate_hand(hand)
  ...
  if is_busted?(score) && how_many_aces_in_hand?(hand) > 0
...
def get_card_ranking(card)
  ...
  if card_is_an_ace_value?(card_value)
    card_ranking = ACE_RANKING_HIGH
  elsif card_is_a_face_value?(card_value)
    card_ranking = FACE_RANKING
  else
    card_ranking = convert_card_value_to_its_numerical_ranking(card_value)
  end
  card_ranking
end
...
def adjust_score_for_aces(hand, score)
  ...
  how_many_aces_in_hand?(hand).times do
    updated_score -= points_to_deduct unless !is_busted?(updated_score)
  end
  updated_score
end
...
def player_turn(player_hand, dealer_hand, score_tracker, deck)
  ...
  loop do
    ...
    display_person_hand_and_score(dealer_hand, DEALER_NAME, score_tracker)
    prompt "\n"
    break unless !is_busted?(get_score(score_tracker, PLAYER_NAME))
  end
end
...
def dealer_wins?(score_tracker)
  ...
  is_busted?(player_score) || dealer_score > player_score
end
...
def dealer_turn(dealer_hand, player_hand, score_tracker, deck)
  ...
  loop do
    ...
    display_person_hand_and_score(dealer_hand, DEALER_NAME, score_tracker)
    prompt "\n"
    break unless !dealer_stops?(score_tracker) && !is_busted?(get_score(score_tracker, DEALER_NAME))                   
  end
end
...
def display_round_results(score_tracker)
  ...
  if player_score == dealer_score
    prompt "It's a tie!"
  elsif is_busted?(dealer_score)
    prompt "#{PLAYER_NAME} have won! #{DEALER_NAME} has busted."
  elsif is_busted?(player_score)
